...
`757c09fca86adf9e3d5899952db3d379e09413f6`. `manifest.json` is the
...
| `NonGravitationalParameters`, Marsden constants, array projections, null construction, and orbit presence/strip helpers | typed quivr/static-data compatibility veneer; force-domain validation belongs to propagators | nested Arrow, mixed/null, validation, and opt-out tests |
| `Orbits.coordinates_to` and representation aliases | thin veneer over the one-crossing Rust coordinate/covariance transform, preserving the complete 6D/9D covariance | mixed representation and 9D transform tests |
...
| `VariantOrbits.create`, `collapse`, `collapse_by_object_id` | one Arrow crossing; Rust owns dimension-6/9 sampling, source-row parameter inheritance, collapse grouping, solved-state means/covariances, validation, and finished batches; Python supplies RNG/UUID compatibility inputs | statistical/fixed parity, 9D reconstruction/collapse, and native timing |
...
| NEOCC, Scout, and SBDB | one Rust HTTP/parse/normalization/Arrow crossing; NEOCC/SBDB own supported A1/A2/A3 covariance mapping and warning products for marginalized/invalid solve-for metadata; Scout additionally owns strict API-signature lifecycle errors and `file=mpc` Obs80 snapshot/hash/metadata assembly | recorded retry/missing/schema/non-grav warning/snapshot tests, native processing timing, live smoke, clean-room acceptance |
