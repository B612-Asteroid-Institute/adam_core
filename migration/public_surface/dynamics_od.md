...
| `departure_spherical_coordinates` | Rust-1x Arrow facade owns normalization, Cartesian-to-spherical/frame transform, metadata, and output assembly; Python preserves assertion/warning compatibility and wraps the batch | existing behavior suite; pinned-main fixture pending | yes | Implementation and Rust-owned timing complete in the active `.37.3.5` slice; frozen baseline artifact remains before bead closure. |
