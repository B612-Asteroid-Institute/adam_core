...
Last updated: 2026-05-07 (RM-P1-020 photometry triaged: predict_magnitudes structural; awaiting decision)
...
- [ ] RM-P1-020: resolve the remaining no-waiver 1.2x single-thread speed rows. Repeated targeted cached-legacy runs (4 reps) and fresh full artifacts give: `photometry.predict_magnitudes` large-n consistently 0.94-0.99 p50 / 0.98-1.11 p95 — confirmed structural JAX/XLA bandpass-gather fusion advantage; `photometry.calculate_apparent_magnitude_v` large-n 1.15-1.23 p50 / 1.22-1.36 p95 — borderline at the 1.2x gate, variance-driven; `dynamics.generate_ephemeris_2body` small-n 1.17-1.24 p50 — borderline; `dynamics.add_light_time`, chi2, calcGauss, transform, and other earlier outliers pass at gate. Multi-thread production scaling artifact (`parity_speed_cold_warm_multithread.json`) shows large-n photometry now 1.85-3.27x p50 / 1.45-3.00x p95 including `predict_magnitudes` 2.23x p50 / 1.69x p95. Awaiting user decision between (a) structural-acceptance for `predict_magnitudes` with the multi-thread artifact as production context, (b) explicit approval for an approximation/polynomial plan with endpoint parity checks, or (c) further low-risk implementation work despite previously exhausted variants.
