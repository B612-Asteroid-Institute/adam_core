...
Last updated: 2026-05-07 (canonical Rust-vs-legacy parity gate switched to multi-thread for production-realistic comparison)
...
- [ ] RM-P1-020: resolve the remaining no-waiver 1.2x speed rows under the new multi-thread canonical gate. Empirical finding 2026-05-07: legacy JAX/XLA on macOS Apple Silicon cannot be reliably constrained from Python env vars, so the historical single-thread parity gate was an asymmetric Rust-1-core vs JAX-uncapped comparison for XLA-vectorized kernels and has been dropped from canonical governance. Single-thread Rayon remains only as the Rust-only regression metric in `pdm run rust-latency-gate`. After switching parity-main / parity-speed-cold to multi-thread on both sides, the previously red large-n photometry rows clear: `predict_magnitudes` 2.14x p50 / 1.48x p95, `calculate_apparent_magnitude_v` 3.04x / 2.91x, fused 3.26x / 2.16x, phase 2.42x / 1.38x. Remaining red rows are tiny-n/small-n Rayon-spawn overhead misses (`cartesian_to_geodetic`, `cartesian_to_cometary`, `residuals.calculate_chi2`, `propagate_2body` at tiny-n; `orbits.classify_orbits` at small-n) — fix with serial dispatch thresholds for n below a per-kernel break-even rather than algorithm changes.
