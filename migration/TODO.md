...
- [ ] RM-STANDALONE-006A: split/factor the large `propagation.rs` module after the trait boundary has settled, without forcing a new crate until explicitly approved.
...
- [ ] RM-STANDALONE-006C: add first-class Rust variant propagation metadata and wiring. The data-model slice (`VariantId`, `OrbitVariantBatch`, weights, and covariance weights) is started; remaining work is to carry variant metadata through typed `PropagationRequest`/`PropagationResult` instead of representing variants only as repeated orbit rows.
- [ ] RM-STANDALONE-006D: enrich `PropagationDiagnostics` with backend convergence details, iteration counts where available, explicit failure codes, and tests for setup-error vs row-failure behavior.
- [ ] RM-STANDALONE-006E: add adapter/provider integration tests for non-TDB propagation inputs and Python/quivr mapping through the explicit `TimeScaleProvider` boundary.
- [ ] RM-STANDALONE-006F: evaluate a reusable propagation execution context/Rayon-pool strategy for hot production loops; keep the current simple per-call `thread_limit` path unless measurements show pool-build overhead matters.
