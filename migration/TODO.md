...
Last updated: 2026-05-19 (RM-STANDALONE-006 typed propagation hardening is in reviewer-remediation: Rust-side validation/diagnostics/benchmark framing are landed, `propagation.rs` is split into focused modules, and Python/quivr adapter parity is tracked separately as blocked W12 adapter work)
...
- [x] RM-WD3-001 centralized dynamics Ray usage behind `ParallelBackend`, closed the dynamics-only steps, and deferred ASSIST/n-body surfaces until the Rust n-body line moves via a safe `assist-sys`/`rebound-sys` wrapper plus traits.
...
- Branch `rust-migration-waves-d-e` now contains the Wave D/E Rust migration plus the standalone Rust foundation slices through RM-STANDALONE-006F. Current reviewer-remediation scope is RM-STANDALONE-006A split/clarity and planning cleanup, not RM-STANDALONE-007 n-body implementation or the broad RM-PERF-001 benchmark-action audit.
- After this cleanup, the next substantial standalone implementation is RM-STANDALONE-006G (`generate_ephemeris<P: Propagator>`). Python/quivr typed propagation parity is W12 adapter work and remains blocked until a typed PyO3 propagation adapter exists.
- RM-STANDALONE-007 code must not start until the ASSIST/REBOUND GPL/package boundary is explicitly decided.
...
- [x] RM-STANDALONE-002: Rust-native data model RFC is captured in [`rust_native_data_model_rfc_2026-05-15.md`](rust_native_data_model_rfc_2026-05-15.md), covering canonical Rust schemas, ERFA/SOFA-first time strategy with Rust-native replacement path, pluggable ASSIST-compatible propagation via a safe `assist-sys`/`rebound-sys` wrapper, covariance, variants, chunking, provenance, network clients, plotting/product outputs, and Arrow/Python adapters.
...
- [ ] RM-STANDALONE-007A: decide the ASSIST/REBOUND GPL/package boundary before RM-STANDALONE-007 code lands. Current preferred direction is a separate optional crate/package boundary mirroring the Python `adam-assist` separation unless explicitly changed.
