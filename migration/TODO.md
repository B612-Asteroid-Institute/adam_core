...
- [ ] RM-STANDALONE-002: write the Rust-native data model RFC next. Scope decisions are resolved: Rust schemas are canonical; time starts with ERFA/SOFA FFI plus a Rust-native replacement outline; `assist-rs` is preferred behind a pluggable backend; network clients and plotting/product outputs are in first standalone scope, with plotting likely requiring a data/spec-oriented approach.
