"""Constitutional parity gate orchestrator.
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Constitutional rust-vs-legacy parity + speedup gate."
    )
    p.add_argument("--apis", nargs="*", default=None,
                   help="Specific API ids (default: ALL).")
    p.add_argument("--fuzz-seeds", type=int, default=8,
                   help="Random seeds per API for parity fuzz (default: 8).")
    p.add_argument("--fuzz-n", type=int, default=128,
                   help="Workload size per parity-fuzz seed (default: 128).")
    p.add_argument("--speed-n", type=int, default=2000,
                   help="Workload size for speed gate (default: 2000).")
    p.add_argument("--speed-reps", type=int, default=7,
                   help="Speed gate reps (default: 7).")
    p.add_argument("--speed-warmup", type=int, default=1,
                   help="Speed gate warmup reps (default: 1).")
    p.add_argument("--base-seed", type=int, default=20260425,
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not args.skip_fuzz:
        ...
        fuzz_results = parity_fuzz.fuzz_all(
            api_ids, seeds=args.fuzz_seeds, n=args.fuzz_n,
    ...
    if not args.skip_speed:
        print()
        print("=" * 72)
        print("SPEEDUP GATE (≥1.2× rust vs legacy)")
        print("=" * 72)
        speed_results = parity_speed.measure_all(
            api_ids, n=args.speed_n, reps=args.speed_reps,
            warmup=args.speed_warmup, seed=args.base_seed,
        )
