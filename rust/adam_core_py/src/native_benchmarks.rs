...
use adam_core_rs_coords::{
    ...
    cartesian_to_spherical_flat6, classify_orbits_flat, cometary_to_cartesian_flat6,
    keplerian_to_cartesian_flat6, rotate_cartesian_time_varying_flat6,
    spherical_to_cartesian_flat6, weighted_covariance_flat, weighted_mean_flat,
};
...
fn flat6(array: &PyReadonlyArray2<'_, f64>) -> PyResult<(Vec<f64>, usize)> {
...
fn benchmark_rotate_cartesian_time_varying_native(
    coords: PyReadonlyArray2<'_, f64>,
    time_index: PyReadonlyArray1<'_, i64>,
    ...
    let covariances = covariance_view
        .as_slice()
        .ok_or_else(|| PyValueError::new_err("covariances must be contiguous"))?;
    let indices: Vec<usize> = time_index
        .as_slice()?
        .iter()
        .map(|&value| {
            usize::try_from(value)
                .map_err(|_| PyValueError::new_err("time_index must be non-negative"))
        })
        .collect::<PyResult<_>>()?;
    if indices.len() != rows {
        return Err(PyValueError::new_err("time_index must have length N"));
    }
    ...
    let matrices = matrix_view
        .as_slice()
        .ok_or_else(|| PyValueError::new_err("matrices must be contiguous"))?;
    samples(reps, trials, warmup, || {
        black_box(
            rotate_cartesian_time_varying_flat6(&coords, covariances, &indices, matrices)
...
fn benchmark_calculate_chi2_native(
    residuals: PyReadonlyArray2<'_, f64>,
    ...
    let residuals = residual_view
        .as_slice()
        .ok_or_else(|| PyValueError::new_err("residuals must be contiguous"))?;
    let covariance_view = covariances.as_array();
    if covariance_view.shape() != [n, d, d] {
        return Err(PyValueError::new_err(
            "covariances must have shape (N, D, D)",
        ));
    }
    let covariances = covariance_view
        .as_slice()
    ...
    samples(reps, trials, warmup, || {
        black_box(
            calculate_chi2_flat(residuals, covariances, n, d)
...
fn benchmark_bound_longitude_residuals_native(
    ...
    let residual_view = residuals.as_array();
    if observed_view.shape() != residual_view.shape() {
        return Err(PyValueError::new_err("shapes must match"));
    }
...
fn benchmark_apply_cosine_latitude_correction_native(
    ...
    let d = residual_view.ncols();
    if lat.len() != n {
        return Err(PyValueError::new_err("lat must have length N"));
    }
    let residuals = residual_view
        .as_slice()
        .ok_or_else(|| PyValueError::new_err("residuals must be contiguous"))?;
    ...
    samples(reps, trials, warmup, || {
        ...
        apply_cosine_latitude_correction_flat(
            lat,
            &mut residual_output,
            &mut covariance_output,
            n,
            d,
        );
...
fn benchmark_weighted_mean_native(
    ...
    if weights.len() != n {
        return Err(PyValueError::new_err("weights must have length N"));
    }
    samples(reps, trials, warmup, || {
...
fn benchmark_weighted_covariance_native(
    ...
    let d = view.ncols();
    if mean.len() != d {
        return Err(PyValueError::new_err("mean must have length D"));
    }
    let values = view
        .as_slice()
        .ok_or_else(|| PyValueError::new_err("samples must be contiguous"))?;
