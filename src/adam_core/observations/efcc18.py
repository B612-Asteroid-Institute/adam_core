...
The table parse, the HEALPix tile lookup (the Rust port of healpy's
``ang2pix`` in the ring scheme) and the correction arithmetic run in the Rust
backend (``adam_core_rs_coords::efcc18``); this module keeps the file
handling: locating, downloading, verifying and caching ``bias.dat``.
...
#: zero correction. Mirrors the Rust ``MPC_ASTCAT_TO_EFCC18`` table.
...
_EXPECTED_SHAPE = (EFCC18_N_TILES, EFCC18_N_CATALOGS, 4)
...
def load_efcc18_biases(
    ...
    `EFCC18_CATALOG_CODES`. The text is parsed in the Rust backend.
    ...
    if (
        ...
        if cached.shape == _EXPECTED_SHAPE:
    ...
    bias_table = np.ascontiguousarray(
        _rust_native.efcc18_parse_bias_dat(bias_dat_path.read_text()),
        dtype=np.float32,
    )
...
def ra_dec_to_healpix(
    ...
    centres are the ring-scheme pixel centres for all 49152 tiles (and
    ...
    unrelated tile; see ``test_tile_ordering_matches_jpl_tiles_dat``. The
    lookup runs in the Rust port of healpy's ``ang2pix``.
    """
    from adam_core import _rust_native

    ra = np.ascontiguousarray(np.atleast_1d(ra_deg), dtype=np.float64)
    dec = np.ascontiguousarray(np.atleast_1d(dec_deg), dtype=np.float64)
    return np.asarray(
        _rust_native.efcc18_ra_dec_to_healpix_numpy(ra, dec), dtype=np.int64
    )
...
def _catalog_columns(
    ...
    return np.asarray(
        _rust_native.efcc18_catalog_columns_numpy(list(astcats), list(exclude_astcats)),
        dtype=np.int64,
    )
...
def compute_efcc18_corrections(
    ...
    exclude = list(exclude_astcats)
    columns = _catalog_columns(astcats, exclude)
    ...
    if bias_table.shape != _EXPECTED_SHAPE:
        raise ValueError(
            f"bias_table has shape {bias_table.shape}, expected {_EXPECTED_SHAPE}"
        )
    return np.asarray(
        _rust_native.efcc18_corrections_numpy(
            np.ascontiguousarray(ra),
            np.ascontiguousarray(dec),
            list(astcats),
            np.ascontiguousarray(jd),
            np.ascontiguousarray(bias_table, dtype=np.float32),
            exclude,
        ),
        dtype=np.float64,
    )
