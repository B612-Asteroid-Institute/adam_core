...
class TestRaysFromDetections:
    ...
    def test_no_matching_exposures(self):
        ...
        detections = PointSourceDetections.from_kwargs(
            ...
            time=times,
            ra=[0.0], dec=[0.0],
            ra_sigma=[0.1], dec_sigma=[0.1],
            mag=[20.0], mag_sigma=[0.1],
        )
    ...
    def test_multiple_observatories(self):
        ...
        assert not np.allclose(obs_x[0], obs_x[1], atol=1e-6) or \
               not np.allclose(obs_y[0], obs_y[1], atol=1e-6) or \
               not np.allclose(obs_z[0], obs_z[1], atol=1e-6)
        assert not np.allclose(obs_x[1], obs_x[2], atol=1e-6) or \
               not np.allclose(obs_y[1], obs_y[2], atol=1e-6) or \
               not np.allclose(obs_z[1], obs_z[2], atol=1e-6)
...
class TestIntegration:
    ...
    def test_full_pipeline_with_realistic_data(self):
        ...
        n_detections = 10
        times = Timestamp.from_mjd(np.linspace(59000.0, 59001.0, n_detections), scale="tdb")
        
        # Create exposures with varied observatories
        exposure_ids = [f"exp_{i}" for i in range(n_detections)]
        observatory_codes = ["X05", "G96", "F51", "703", "691"] * 2  # Cycle through observatories
