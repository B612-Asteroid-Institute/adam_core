...
The tile lookup is the Rust port of healpy's ring-scheme ``ang2pix``; healpy
itself is an optional test dependency used only as an independent oracle.
"""
...
except ImportError:  # pragma: no cover - healpy is an optional oracle
    hp = None
...
class TestHealpix:
    ...
    def test_tile_centers_round_trip(self) -> None:
        ...
        if hp is None:
            pytest.skip("healpy not installed (independent HEALPix oracle)")
    ...
    def test_matches_healpy_ring_oracle(self) -> None:
        # The Rust ang2pix port must agree with healpy's ring scheme at
        # nside=64 on a dense sky sample, including the polar caps.
        if hp is None:
            pytest.skip("healpy not installed (independent HEALPix oracle)")
        rng = np.random.default_rng(7)
        ra = rng.uniform(0.0, 360.0, 20000)
        dec = np.degrees(np.arcsin(rng.uniform(-1.0, 1.0, 20000)))
        ra = np.concatenate([ra, [0.0, 359.999999, 180.0, 45.0]])
        dec = np.concatenate([dec, [89.9999, -89.9999, 0.0, 66.0]])
        expected = hp.ang2pix(
            EFCC18_NSIDE, np.radians(90.0 - dec), np.radians(ra), nest=False
        )
        np.testing.assert_array_equal(ra_dec_to_healpix(ra, dec), expected)
...
class TestCorrections:
    ...
    def test_proper_motion_term(self) -> None:
        table = np.zeros((EFCC18_N_TILES, EFCC18_N_CATALOGS, 4), dtype=np.float32)
        # Centre of ring tile 0 (JPL tiles.dat: RA 0.785398 rad, Dec 1.558038 rad)
        ra, dec = float(np.degrees(0.785398)), float(np.degrees(1.558038))
        assert int(ra_dec_to_healpix([ra], [dec])[0]) == 0
...
def test_tile_ordering_matches_jpl_tiles_dat() -> None:
    ...
    nested = _rust_native.healpix_ang2pix_lonlat_numpy(
        EFCC18_NSIDE,
    ...
    if hp is not None:
        np.testing.assert_array_equal(
            nested,
            hp.ang2pix(EFCC18_NSIDE, np.radians(90.0 - dec), np.radians(ra), nest=True),
        )
