...
def test_generate_impact_visualization_data(time_step, time_range, max_processes):
    ...
    time_span_days = (np.max(impact_times) - np.min(impact_times)) + (time_range / 60 / 24 * 2)
...
def test_plot_impact_simulation(time_step, time_range):
    ...
    test_cases = [
        {"title": "Test Impact Simulation", "logo": True, "sample_impactors": 0.5, "sample_non_impactors": 0.5},
        {"title": None, "logo": False, "show_impacting": False, "show_non_impacting": True},
        {"title": "Minimal Test", "grid": False, "show_earth": True, "show_moon": False}
    ]
    ...
    for params in test_cases:
        try:
            ...
            if "No Earth impacts found" in str(e):
                pytest.skip("No Earth impacts in test data, skipping risk corridor test")
            else:
                raise 