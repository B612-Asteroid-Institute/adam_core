...
class TestCartesianValuesConversion:
    ...
    def test_convert_cartesian_values_km_to_au(self):
        ...
        values_km = np.array([
            ...
            [2*KM_P_AU, -0.5*KM_P_AU, KM_P_AU, 0.1*KM_P_AU/S_P_DAY, 0.0, -0.05*KM_P_AU/S_P_DAY]
        ])
...
class TestCovarianceConversion:
    ...
    def test_full_covariance_conversion(self):
        ...
        assert np.isclose(result_km[0, 3, 3], 0.0001 * (KM_P_AU/S_P_DAY)**2)  # vx variance
        assert np.isclose(result_km[0, 4, 4], 0.0002 * (KM_P_AU/S_P_DAY)**2)  # vy variance
        ...
        assert np.isclose(result_km[0, 0, 3], 0.001 * KM_P_AU * KM_P_AU/S_P_DAY)  # x-vx correlation
        assert np.isclose(result_km[0, 3, 0], 0.001 * KM_P_AU * KM_P_AU/S_P_DAY)  # symmetry
