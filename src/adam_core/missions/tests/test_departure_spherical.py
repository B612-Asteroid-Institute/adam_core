...
class TestDepartureSphericalCoordinates:
    ...
    def test_error_conditions(self):
        ...
        empty_vy = np.array([])
        empty_vz = np.array([])
        with pytest.raises(AssertionError, match="At least one departure vector is required"):
            departure_spherical_coordinates(
    ...
    def test_coordinate_directions_simple_cases(self):
        ...
        result_x = departure_spherical_coordinates(
            OriginCodes.EARTH, times, "ecliptic", 
    ...
    def test_different_input_frames(self):
        ...
        assert (not np.allclose(lon_ecliptic, lon_equatorial, rtol=1e-6) or 
                not np.allclose(lat_ecliptic, lat_equatorial, rtol=1e-6))
    ...
    def test_interplanetary_mission_example(self):
        ...
        for origin in [OriginCodes.EARTH, OriginCodes.MARS]:
            ...
            vx = np.array([target_x * 10.0])  # Scale doesn't matter due to normalization
