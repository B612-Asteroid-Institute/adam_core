...
def test_porkchop_different_time_grids():
    ...
    departure_times = Timestamp.from_mjd([60000, 60005, 60010, 60015, 60020], scale="tdb")
    departure_coords = CartesianCoordinates.from_kwargs(
        ...
        vy=[0.017, 0.016, 0.015, 0.014, 0.013],  # Earth orbital velocity ~17 km/s -> 0.017 AU/day
        vz=[0.0, 0.001, 0.002, 0.003, 0.004],
        frame="ecliptic",
    )
    
    # Create arrival coordinates with a DIFFERENT time grid (shifted and different spacing)
    arrival_times = Timestamp.from_mjd([60030, 60040, 60050, 60060], scale="tdb")  # Different times!
    ...
    assert np.all(dep_mjd < arr_mjd), "All departure times should be before arrival times"
    ...
    assert len(results) == expected_combinations, f"Expected {expected_combinations} combinations, got {len(results)}"
...
def test_porkchop_overlapping_time_grids():
    ...
    arrival_times = Timestamp.from_mjd([60010, 60013, 60016, 60019, 60022, 60025], scale="tdb")
    arrival_coords = CartesianCoordinates.from_kwargs(
        time=arrival_times,
    ...
    assert len(results) == valid_count, f"Expected {valid_count} valid combinations, got {len(results)}"
...
def test_porkchop_problematic_case_that_old_version_would_fail():
    ...
    departure_times = Timestamp.from_mjd([60020, 60021, 60022], scale="tdb")  # Later times
    departure_coords = CartesianCoordinates.from_kwargs(
        time=departure_times,
        x=[1.0, 1.1, 1.2],
        y=[0.0, 0.1, 0.2],
    ...
    arrival_times = Timestamp.from_mjd([60000, 60001, 60002], scale="tdb")  # Earlier times
    arrival_coords = CartesianCoordinates.from_kwargs(
        time=arrival_times,
        x=[1.5, 1.6, 1.7],
        y=[0.5, 0.6, 0.7],
    ...
    assert len(results) == 0, f"Expected 0 valid combinations (departure after arrival), got {len(results)}"
