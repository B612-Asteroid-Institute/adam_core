...
def _compute_orbital_basis(
    ...
    for i in range(len(kep_coords)):
        ...
        if abs(p_norm - 1.0) > tolerance or abs(q_norm - 1.0) > tolerance or abs(n_norm - 1.0) > tolerance:
            logger.warning(f"Basis vectors not unit length for orbit {i}: |p|={p_norm:.2e}, |q|={q_norm:.2e}, |n|={n_norm:.2e}")
            # Renormalize
            p_hat[i] /= p_norm
            q_hat[i] /= q_norm  
            n_hat[i] /= n_norm
        ...
        if abs(pq_dot) > tolerance or abs(pn_dot) > tolerance or abs(qn_dot) > tolerance:
            logger.warning(f"Basis vectors not orthogonal for orbit {i}: p·q={pq_dot:.2e}, p·n={pn_dot:.2e}, q·n={qn_dot:.2e}")
...
def sample_ellipse_adaptive(
    ...
    for i, orbit_id in enumerate(orbit_ids):
        ...
        n_initial = 1024
        f_samples = np.linspace(0, 2*np.pi, n_initial + 1)[:-1]  # Exclude 2π (same as 0)
        
        # Iteratively refine segments that violate chord constraint
        segments_to_check = [(f_samples[j], f_samples[(j+1) % len(f_samples)]) for j in range(len(f_samples))]
        final_f_values = set(f_samples)
        
        iteration = 0
        max_iterations = 10
        
        while segments_to_check and len(final_f_values) < max_segments_per_orbit and iteration < max_iterations:
            new_segments = []
            
            for f0, f1 in segments_to_check:
                # Compute positions at segment endpoints
                pos0 = _sample_ellipse_points(
                    np.array([a[i]]), np.array([e[i]]),
                    p_hat[i:i+1], q_hat[i:i+1], 
                    np.array([[f0]])
                )[0, 0]
        ...
        if len(final_f_values) >= max_segments_per_orbit:
            logger.warning(f"Orbit {orbit_id} hit segment limit {max_segments_per_orbit}")
