...
def test_query_horizons_chunking():
    ...
    for coord_type in coordinate_types:
        ...
        np.testing.assert_allclose(orbits_batched.coordinates.r, orbits_batched_none.coordinates.r, rtol=1e-15)
        ...
        assert len(orbits_batched) == 100, f"Expected 100 orbits for {coord_type}, got {len(orbits_batched)}"
