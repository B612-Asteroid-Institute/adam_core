...
@pytest.fixture(scope="session")
def index_optimal(
    _fixture_cache_root: Path, orbits_synthetic_stratified_ci: Orbits
) -> BVHIndex:
    """BVH index built from persisted segments with optimal params; atomic write to dir."""
    # Ensure segments for index parameters exist (sampling params 5.0/512 as before)
    seg_root = Path(_fixture_cache_root) / "segments"
    _ensure_dir(seg_root)
    seg_path = seg_root / "synthetic_stratified_ci.parquet"
    seg_lock = seg_root / ".lock"
    from adam_core.orbits.polyline import OrbitPolylineSegments

    with _file_lock(seg_lock):
        if seg_path.exists():
    ...
    with _file_lock(lock):
        ...
        index = build_bvh_index_from_segments(segments, max_leaf_size=64)
        tmp_dir = idx_root / "bvh_index.tmp"
        if tmp_dir.exists():
