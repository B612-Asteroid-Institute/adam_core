...
def process_ray_batch_remote(
    ...
    bvh_arrays = (
        ray.get(bvh_arrays_ref) if isinstance(bvh_arrays_ref, ray.ObjectRef) else bvh_arrays_ref
    )
    segments_soa = (
        ray.get(segments_soa_ref) if isinstance(segments_soa_ref, ray.ObjectRef) else segments_soa_ref
    )
    orbit_mapping = (
        ray.get(orbit_mapping_ref) if isinstance(orbit_mapping_ref, ray.ObjectRef) else orbit_mapping_ref
    )
