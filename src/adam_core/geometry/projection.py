...
def project_ray_to_orbital_plane(
    ...
    t = jnp.where(
        jnp.abs(n_dot_d) > 1e-12,
        -n_dot_w / n_dot_d,
        0.0  # If parallel, use ray origin
    )
...
def ellipse_snap_distance_multi_seed(
    ...
    This uses multiple seed points and Newton's method to find multiple local minima,
    which can occur near nodes or in ambiguous geometries.
    ...
    max_iterations : int, default=10
        Maximum Newton iterations per seed
    ...
    -------
    distances : jax.Array
        Distances from point to ellipse for each candidate (max_k,), padded with inf
    E_values : jax.Array
        Eccentric anomalies of candidates (max_k,), padded with NaN
    valid_mask : jax.Array
        Boolean mask indicating which candidates are valid (max_k,)
    """
    # Use a cached JIT-compiled version for common max_k values
    if max_k == 1:
        return _ellipse_snap_multi_k1(point_2d, a, e, max_iterations, dedupe_angle_tol)
    elif max_k == 3:
        return _ellipse_snap_multi_k3(point_2d, a, e, max_iterations, dedupe_angle_tol)
    else:
        # Fallback to max_k=3 and truncate
        distances, E_values, valid_mask = _ellipse_snap_multi_k3(point_2d, a, e, max_iterations, dedupe_angle_tol)
        if max_k < 3:
            return distances[:max_k], E_values[:max_k], valid_mask[:max_k]
        else:
            # Pad to requested size
            pad_size = max_k - 3
            distances = jnp.concatenate([distances, jnp.full(pad_size, jnp.inf)])
            E_values = jnp.concatenate([E_values, jnp.full(pad_size, jnp.nan)])
            valid_mask = jnp.concatenate([valid_mask, jnp.zeros(pad_size, dtype=bool)])
            return distances, E_values, valid_mask


@jax.jit
def _ellipse_snap_multi_k1(
    point_2d: jax.Array,
    a: jax.Array,
    e: jax.Array,
    max_iterations: int,
    ...
    return _ellipse_snap_multi_kernel(point_2d, a, e, 1, max_iterations, dedupe_angle_tol)


@jax.jit
def _ellipse_snap_multi_k3(
    point_2d: jax.Array,
    a: jax.Array,
    e: jax.Array,
    max_iterations: int,
    ...
    return _ellipse_snap_multi_kernel(point_2d, a, e, 3, max_iterations, dedupe_angle_tol)
