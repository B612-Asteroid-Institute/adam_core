...
class BVHArrays:
    ...
    def get_leaf_primitives(self, node_idx: int) -> tuple[jax.Array, jax.Array, jax.Array]:
        """
        ...
        return (self.prim_row_index[first:slice_end],
...
class AnomalyLabelsSOA:
    ...
    def validate_structure(self) -> None:
        ...
        arrays_to_check = [
            self.det_indices, self.orbit_indices, self.seg_ids, self.variant_ids,
            self.f_rad, self.E_rad, self.M_rad, self.mean_motion_rad_day, self.r_au,
            self.snap_error, self.plane_distance_au, self.curvature_hint, self.mask
        ]
        ...
        # Check dtypes
        int_arrays = [self.det_indices, self.orbit_indices, self.seg_ids, self.variant_ids]
        for arr in int_arrays:
            assert arr.dtype == jnp.int32, f"Integer array has dtype {arr.dtype}"
        
        float_arrays = [
            self.f_rad, self.E_rad, self.M_rad, self.mean_motion_rad_day, self.r_au,
            self.snap_error, self.plane_distance_au, self.curvature_hint
        ]
...
jax.tree_util.register_pytree_node(
    BVHArrays,
    lambda bvh: (
        (bvh.nodes_min, bvh.nodes_max, bvh.left_child, bvh.right_child, 
         bvh.is_leaf, bvh.first_prim, bvh.prim_count, bvh.prim_row_index,
         bvh.orbit_id_index, bvh.prim_seg_ids), 
        None
    ),
...
jax.tree_util.register_pytree_node(
    HitsSOA,
    lambda hits: (
        (hits.det_indices, hits.orbit_indices, hits.seg_ids, 
         hits.leaf_ids, hits.distances_au),
        None
    ),
...
jax.tree_util.register_pytree_node(
    AnomalyLabelsSOA,
    lambda labels: (
        (labels.det_indices, labels.orbit_indices, labels.seg_ids, labels.variant_ids,
         labels.f_rad, labels.E_rad, labels.M_rad, labels.mean_motion_rad_day, labels.r_au,
         labels.snap_error, labels.plane_distance_au, labels.curvature_hint, labels.mask),
        None
    ),
