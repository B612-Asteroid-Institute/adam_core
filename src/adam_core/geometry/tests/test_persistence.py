...
class TestBVHArraysPersistence:
    ...
    def test_save_load_roundtrip(self, sample_bvh_arrays):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            np.testing.assert_array_equal(
                sample_bvh_arrays.left_child, loaded.left_child
            )
            np.testing.assert_array_equal(
                sample_bvh_arrays.right_child, loaded.right_child
            )
            np.testing.assert_array_equal(sample_bvh_arrays.is_leaf, loaded.is_leaf)
            np.testing.assert_array_equal(
                sample_bvh_arrays.first_prim, loaded.first_prim
            )
            np.testing.assert_array_equal(
                sample_bvh_arrays.prim_count, loaded.prim_count
            )
            np.testing.assert_array_equal(
                sample_bvh_arrays.prim_row_index, loaded.prim_row_index
            )
            np.testing.assert_array_equal(
                sample_bvh_arrays.orbit_id_index, loaded.orbit_id_index
            )
            np.testing.assert_array_equal(
                sample_bvh_arrays.prim_seg_ids, loaded.prim_seg_ids
            )
    ...
    def test_memory_mapping(self, sample_bvh_arrays):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            np.testing.assert_array_equal(
                loaded_mmap.prim_row_index, loaded_mem.prim_row_index
            )
    ...
    def test_device_placement(self, sample_bvh_arrays):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            np.testing.assert_array_equal(
                sample_bvh_arrays.nodes_min, loaded_cpu.nodes_min
            )
...
class TestSegmentsSOAPersistence:
    ...
    def test_save_load_roundtrip_no_normals(self, sample_segments_soa_no_normals):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            np.testing.assert_array_equal(
                sample_segments_soa_no_normals.r_mid_au, loaded.r_mid_au
            )
...
class TestPersistenceIntegration:
    ...
    def test_integration_with_bvh_conversion(self):
        ...
        orbit_mapping = OrbitIdMapping.from_orbit_ids(
            segments_with_aabbs.orbit_id.to_pylist()
        )
