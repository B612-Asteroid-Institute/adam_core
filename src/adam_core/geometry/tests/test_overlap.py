...
class TestQueryBvh:
    ...
    def test_basic_overlap_detection(self, bvh_index, rays):
        ...
        hits = geometric_overlap(
            segments, rays, guard_arcmin=5.0
        )  # Large guard for testing
...
class TestIntegration:
    ...
    def test_performance_with_many_rays(self):
        ...
        # Create many rays
        n_rays = 100
        ray_times = Timestamp.from_mjd(
            np.linspace(59000.0, 59001.0, n_rays), scale="tdb"
        )
...
class TestQueryBvhParallel:
    ...
    def test_parallel_vs_serial_consistency(self, max_processes):
        ...
        if len(hits_serial) > 0:
            serial_keys = sorted(
                [hit_key(hits_serial, i) for i in range(len(hits_serial))]
            )
            parallel_keys = sorted(
                [hit_key(hits_parallel, i) for i in range(len(hits_parallel))]
            )
