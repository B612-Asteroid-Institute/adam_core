...
class TestAnomalyRemote:
    ...
    def test_remote_empty_placeholder(self):
        """Placeholder: remote batching API was removed; ensure runner still imports."""
        assert True

    def test_label_anomalies_empty(self):
        """Test labeling with empty hits (serial)."""
        from adam_core.geometry.overlap import OverlapHits
        from adam_core.coordinates.cartesian import CartesianCoordinates
        from adam_core.orbits.orbits import Orbits

        empty_hits = OverlapHits.empty()
        empty_orbits = Orbits.from_kwargs(
            orbit_id=[], coordinates=CartesianCoordinates.empty()
        )
        empty_rays = ObservationRays.empty()
        labels = label_anomalies(empty_hits, empty_rays, empty_orbits)
    ...
    def test_query_bvh_parallel_and_labeling_integration(self):
        ...
        from adam_core.geometry.sharded_query_ray import (
            query_manifest_ray as _maybe_absent,
        )
    ...
    def test_parallel_vs_serial_consistency(self):
        ...
        serial_labels = label_anomalies(serial_hits, rays, orbit)

        # Get parallel results via max_processes: query then label
        parallel_hits = query_bvh_jax(bvh, segments_with_aabbs, rays)
        parallel_labels = label_anomalies(parallel_hits, rays, orbit, max_processes=2, chunk_size=2)
