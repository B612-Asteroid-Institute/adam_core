...
class TestClockGating:
    ...
    def test_circular_orbit_clock_gating(self):
        ...
        coords = CartesianCoordinates.from_kwargs(
            x=[1.0], y=[0.0], z=[0.0],  # At periapsis
        ...
        positions, _, _ = compute_orbital_positions_at_times(elements, bases, epoch_mjd, obs_times_j)
    ...
    def test_clock_gating_filtering(self):
        ...
        bases = jnp.array([p_hat, q_hat])
        epoch_mjd = jnp.array(plane_params.t0.mjd().to_numpy()[0])
        obs_times_j = jnp.array([obs_times.mjd().to_numpy()[0], obs_times.mjd().to_numpy()[1]])
        positions, _, _ = compute_orbital_positions_at_times(elements, bases, epoch_mjd, obs_times_j)
        pred = np.array(positions[0])
    ...
    def test_eccentric_orbit(self):
        ...
        coords = CartesianCoordinates.from_kwargs(
            x=[0.5], y=[0.0], z=[0.0],  # At periapsis for a=1, e=0.5
            vx=[0.0], vy=[0.024248], vz=[0.0],  # Velocity at periapsis
            time=times,
            origin=Origin.from_kwargs(code=[OriginCodes.SUN.name]),
            frame="ecliptic",
        )
    ...
    def test_jax_vs_legacy_consistency(self):
        ...
        rays = ObservationRays.from_kwargs(
            ...
            u_x=[0.0, 0.1], u_y=[1.0, 0.9], u_z=[0.0, 0.0],
        )
