...
def _merge_overlap_hits(hits_list: list[OverlapHits]) -> OverlapHits:
    ...
    sorted_hits = combined_hits.sort_by(
        [("det_id", "ascending"), ("distance_au", "ascending")]
    )
    return sorted_hits
...
def query_manifest_local(
    ...
    for batch_start in range(0, num_rays, ray_batch_size):
        ...
        logger.info(
            f"Processing ray batch {batch_start}-{batch_end} "
        ...
        for shard_idx, meta in enumerate(manifest.shards):
            ...
            if len(shard_hits) > 0:
                ...
                logger.debug(
                    f"Shard {shard_idx}: {len(shard_hits)} hits "
