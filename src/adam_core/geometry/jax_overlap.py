...
from .jax_types import HitsSOA, OrbitIdMapping
...
def query_bvh_jax(
    ...
    ray_origins, ray_directions, observer_distances = rays_to_arrays(
        rays, device=device
    )
    ...
    if fixed_num_rays is not None and fixed_num_rays > orig_num_rays:
        ...
        observer_distances = jax.numpy.concatenate(
            [observer_distances, pad_obs], axis=0
        )
    ...
    if (
        ...
        and fixed_num_rays > orig_num_rays
        and hits_soa.num_hits > 0
    ):
        mask = hits_soa.det_indices < jax.numpy.asarray(
            orig_num_rays, dtype=jax.numpy.int32
        )
